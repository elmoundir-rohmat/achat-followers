...
export default function TikTokCheckoutPage({ onBack, onComplete }: TikTokCheckoutPageProps) {
  ...
  const [orderId] = useState(`TIKTOK-ORDER-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`);
  ...
  const handlePaymentSuccess = async (result: any) => {
    ...
    try {
      ...
      console.error('❌ Erreur lors du traitement SMMA TikTok:', error);
      setSmmaResult({ error: 'Erreur lors du traitement de la commande TikTok' });
    } finally {
  ...
  const handlePaymentError = (error: any) => {
    console.error('❌ Erreur de paiement TikTok:', error);
              ...
              <div className="mt-6 bg-blue-50 border border-blue-200 rounded-lg p-4">
                <div className="flex items-center justify-center">
                  ...
                  <span className="text-blue-700 font-medium">
                    Traitement de votre commande TikTok avec la plateforme SMMA...
                  </span>
                </div>
              </div>
            )}
                ...
                <button
                  ...
                  className={`w-full font-bold py-4 px-6 rounded-xl transition-all duration-300 shadow-lg text-lg flex items-center justify-center ${
                    items.length === 0
                      ...
                      : 'bg-gradient-to-r from-pink-600 to-purple-600 hover:from-pink-700 hover:to-purple-700 text-white transform hover:scale-105 hover:shadow-xl'
                  }`}
                >
                  <CreditCard className="w-6 h-6 mr-3" />
                  {items.length === 0 ? 'Panier vide' : `Finaliser la commande TikTok - ${getTotalPrice().toFixed(2)}€`}
                </button>
              </div>
            </form>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-xl p-8">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-2xl font-bold text-gray-900">
                Résumé de votre commande TikTok
              </h2>
              {items.length > 0 && (
                <button
                    ...
                    <div className="flex justify-between items-center pr-8">
                      <div>
                        <div className="font-semibold text-gray-900">
                          {item.followers.toLocaleString()} followers TikTok {item.followerType === 'french' ? 'français' : 'internationaux'}
                        </div>
                        {item.username && (
                        ...
                        {item.deliveryOption && (
                          <div className="text-xs text-gray-500 mt-1">
                            Livraison: {item.deliveryOption.totalTime}
                            {item.deliveryOption.additionalCost > 0 && ` (+${item.deliveryOption.additionalCost.toFixed(2)}€)`}
                          </div>
                        )}
                      </div>
            ...
            <div className="mt-6 p-4 bg-green-50 rounded-lg">
              ...
              <div className="flex items-center text-green-700 mt-2">
                <div className="w-2 h-2 bg-green-500 rounded-full mr-2"></div>
                <span className="text-sm font-medium">Profils réels TikTok</span>
              </div>
            </div>
          </div>
        </div>
