...
import { PageService, CaptionGeneratorPageData } from '../services/pageService';
...
export default function InstagramCaptionGenerator() {
  ...
  const [pageData, setPageData] = useState<CaptionGeneratorPageData | null>(null);
  ...
  useEffect(() => {
    const loadPageData = async () => {
      try {
        setLoading(true);
        const data = await PageService.getCaptionGeneratorPage();
        setPageData(data);
      } catch (error) {
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        <div className="text-center mb-10">
          <h1 className="text-3xl md:text-4xl font-semibold text-slate-800 mb-4">
            {pageData?.hero?.title || 'Générateur de captions Instagram'}
          </h1>
